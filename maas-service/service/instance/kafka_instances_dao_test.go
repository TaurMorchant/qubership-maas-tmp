...
func TestKafkaInstanceOperations(t *testing.T) {
	...
	dao.WithSharedDao(t, func(baseDao *dao.BaseDaoImpl) {
		...
		notExistedInstance := &model.KafkaInstance{
			Id:           "not_existed_id",
			Addresses:    nil,
			Default:      false,
			MaasProtocol: "PLAINTEXT",
...

		{
			...
			_, err = dao.RemoveInstanceRegistration(ctx, notExistedInstance.Id)
			assert.Error(t, err)
		}

		{
			//test fail update registration for not existed instance
			_, err = dao.UpdateInstanceRegistration(ctx, notExistedInstance)
			assert.Error(t, err)
		}
